...
def getUserID(email):
    try:
        user = session.query(GoogleUser).filter_by(email=email).one()
        return user.id
...
def createUser(login_session):
    ...
    session.add(newUser)
    session.commit()
    user = session.query(GoogleUser).filter_by(
        email=login_session['email']).one()
    return user.id
	
	
def getDbUserId():
    mail = login_session['email']
    user = session.query(GoogleUser).filter_by(email=mail).first()
    return user
	
	
def can_to_edit_delete(user_id):
   print ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
   print user_id
   ...
   print db_user.id
   print ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
   if db_user == None or user_id != db_user.id :
        print("testing.......")
        return True
...
@app.route('/<cat_name>/cat_items.json')
...
def showItems(name):
    ...
    if 'username' not in login_session or \
        ...
        return render_template(
            'withoutLoginCategoryItems.html',
            categories=cat_list,
            chosenCategory=Cat,
            items=items)
    else:
        return render_template(
            'loginCategoryItems.html',
            categories=cat_list,
            chosenCategory=Cat,
            items=items)
...
def CategoryEdit(c_name):
    ...
    if can_to_edit_delete(cat.user_id):
        flash("You can't edit or delete records that you didn't create!")
        return redirect(url_for('showHome'))
    cat = session.query(Category).filter_by(name=c_name).one()
    if request.method == 'POST':
...
def editItem(c_name, i_name):
    ...
    editingItem = session.query(Item).filter_by(
        name=i_name, category=editingItemCategory).one()
    if can_to_edit_delete(editingItem.user_id):
        flash("You can't edit or delete records that you didn't create!")
        return redirect(url_for('showHome'))
    ...
    if request.method == 'POST':
        ...
        return redirect(
            url_for(
                'showItem',
                c_name=editingItemCategory.name,
                i_name=editingItem.name))
    else:
        return render_template(
            'editItem.html',
            categories=categories,
...
def ItemDelete(c_name, i_name):
    ...
    deletingItem = session.query(Item).filter_by(
        name=i_name, category=category).one()
    if can_to_edit_delete(deletingItem.user_id):
        flash("You can't edit or delete records that you didn't create!")
        return redirect(url_for('showHome'))
    if request.method == 'POST':
